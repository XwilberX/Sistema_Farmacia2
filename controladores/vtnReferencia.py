...
"")
    ...
    def showTableW(self, VtnES):
        ...
        if self.EnOsal == 0:
            self.NSalida = self.tableViewReferencia.model().index(indexTableview, 0).data()
            self.fechaR = self.tableViewReferencia.model().index(indexTableview, 1).data()
            self.fechaE = self.tableViewReferencia.model().index(indexTableview, 2).data()
            self.destino = self.tableViewReferencia.model().index(indexTableview, 3).data()
    ...
    def sendReport(self):
        ...
        if self.EnOsal == 0:
            datarow = ['Clave', 'Descripción', 'Presentación', 'Cantidad', 'Lote', 'Destino']
        ...
        if self.EnOsal ==0:
            reportes(dataall, self.destino, self.NSalida, self.fechaR, self.fechaE, observacion, tipo)
