...
class SubWindow(QWidget):
        ...
        try:
            ...
            for i in range(rows):
                for j in range(Column + 3):
                    ...
                    if j != 0 and j < Column:
                        DfReport.loc[i, j] = self.TableSalida.item(i, j).text()
                    ...
                    if j == 9:
                        ...
                        DfReport.loc[i, j] = self.LineAreaSalida.text()
                    if j == 10:
                        DfSalida.loc[i, j] = self.listaLote[i]
                        DfReport.loc[i, j] = self.listaLote[i]
                    if j == 11:
            ...
            DfReport.columns = list_names
            #pasando de dataframe a una lista de listas
            LisReport = [DfReport.columns[:, ].values.astype(str).tolist()] + DfReport.values.tolist()
            ...
            reportes(LisReport, self.LineAreaSalida.text(), Npedidoup, str(FechaEsalida),
                                     str(FechaPsalida), desc)
    ...
    def insertDatosTablaWid(self):
        ...
        if not self.claveid in self.listaId:
            ...
            self.Query = session.query(Clave.descripcion, Clave.presentacion, Farmaco.cantidad, Farmaco.caducidad).join(
                Clave).filter(Farmaco.idFarmaco == self.claveid).one()
    ...
    def inserDatosBtn(self):
        try:
            ...
            if cantidadPuesta <= self.Query[2] and cantidadPuesta != 0:
                ...
                IconoDelete2.addPixmap(QtGui.QPixmap("../imagenes/ic_delete_128_28267.png"), QtGui.QIcon.Normal,
                                       QtGui.QIcon.Off)
    ...
    def TableViewInsertSalida(self):
        ...
        self.q = session.query(Farmaco.idFarmaco, Farmaco.clave_corta, Clave.descripcion, Farmaco.caducidad,
                               Farmaco.lote, Farmaco.cantidad, Farmaco.area, Farmaco.fechaIngreso).join(Clave).filter(
        ...
        self.model.setHorizontalHeaderLabels(
            ['Id farmaco', 'Clave', 'Descripción', 'Caducidad', 'Lote', 'Cantidad', 'Area Almacen', 'Fecha Ingreso'])
        ...
        for h in range(self.numero):
            ...
            if cantiRojo <= 10:
                ...
                self.model.setData(self.model.index(h, 5), QtGui.QBrush(QtGui.QColor(236, 47, 6)),
